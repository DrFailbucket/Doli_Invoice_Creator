...
function renderFieldLibrary() {
  ...
  const selectedPack = fieldPacks.find((pack) => pack.id === activeFieldPackId);
  const sourceFields = fieldLibraryScope === "industry" ? (selectedPack?.fields || []).filter((field) => field.documentTypes.includes(projectState.documentType)) : fieldLibraryScope === "all" ? FIELD_REGISTRY : getFieldsForDocumentType(projectState.documentType);
  const recommendedFields = fieldLibraryScope === "industry" ? [] : getRecommendedFieldsForDocumentType(projectState.documentType).filter((field) => sourceFields.includes(field) && fieldMatchesSearch(field, query));
...
function markUiTranslations() {
  ...
  set(".field-library-scope label:nth-child(1)", "sidebar.forDocumentType"); set(".field-library-scope label:nth-child(2)", "sidebar.allFields"); set(".field-library-scope label:nth-child(3)", "sidebar.industryFields"); set(".field-library-industry", "sidebar.industry"); set(".field-library-search > span", "sidebar.search");
...
dom.fieldLibraryScope.forEach((input) => input.addEventListener("change", (event) => {
  ...
  if (fieldLibraryScope === "industry" && !activeFieldPackId) activeFieldPackId = fieldPacks[0]?.id || "";
  renderFieldPackSelector();
  renderFieldLibrary();
}));
dom.fieldLibraryIndustry.addEventListener("change", (event) => { activeFieldPackId = event.target.value; renderFieldLibrary(); });
