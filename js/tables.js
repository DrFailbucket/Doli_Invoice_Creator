...
const CORE_COLUMNS = [
  ...
  ["line_desc", "Beschreibung", 75, "left", true],
...
function normalizeColumnId(id) {
  if (id === "line_description" || id === "line_fulldesc") return "line_desc";
  return id;
}
...
export function normalizeTableColumns(table) {
  const existingById = new Map((Array.isArray(table.columns) ? table.columns : []).map((column) => [normalizeColumnId(column.id), column]));
...
export function validateTable(table) { const errors = []; const warnings = []; const rowHeight = Number(table.rowHeightMm) || 0; const minRowHeight = Number(table.minRowHeightMm) || 0; if (table.width <= 0) errors.push(t("table.validation.widthPositive", "Tabellenbreite muss gr\u00f6\u00dfer als 0 sein.")); if (table.height <= 0) errors.push(t("table.validation.heightPositive", "Tabellenh\u00f6he muss gr\u00f6\u00dfer als 0 sein.")); if (table.rowMode === "fixed" && rowHeight <= 0) errors.push(t("table.validation.rowHeightPositive", "Zeilenh\u00f6he muss gr\u00f6\u00dfer als 0 sein.")); if (table.rowMode === "dynamic" && minRowHeight <= 0) errors.push(t("table.validation.minHeightPositive", "Mindesth\u00f6he muss gr\u00f6\u00dfer als 0 sein.")); if ((Number(table.cellPaddingHorizontalMm) || 0) < 0 || (Number(table.cellPaddingVerticalMm) || 0) < 0) errors.push(t("table.validation.paddingNonNegative", "Zellen-Innenabstand darf nicht negativ sein.")); if ((Number(table.lineHeight) || 0) <= 0) errors.push(t("table.validation.lineHeightPositive", "Line Height muss gr\u00f6\u00dfer als 0 sein.")); if (table.columns.some((column) => column.widthMm < 3)) errors.push(t("table.validation.columnWidthMin", "Spaltenbreite darf nicht kleiner als 3 mm sein.")); const total = table.columns.reduce((sum, column) => sum + Number(column.widthMm || 0), 0); if (Math.abs(total - table.width) > 0.01) errors.push(t("table.validation.columnsMatch", "Summe der Spaltenbreiten stimmt nicht mit der Tabellenbreite \u00fcberein.")); if (table.x < 0 || table.y < 0 || table.x + table.width > 210 || table.y + table.height > 297) warnings.push(t("table.validation.outsidePage", "Tabelle liegt teilweise au\u00dferhalb der A4-Seite.")); if (table.rowMode === "fixed" && table.height < rowHeight) warnings.push(t("table.validation.rowTooSmall", "Tabellenh\u00f6he ist kleiner als eine Standardzeile.")); return { errors, warnings, maxRows: rowHeight > 0 ? Math.floor(table.height / rowHeight) : 0 }; }

export const TEST_ROWS = [{ line_position: "1", line_desc: "Motor\u00f6l 5W-30", line_quantity: "5,0", line_unit: "Liter", line_unit_price: "12,90 \u20ac", line_total: "64,50 \u20ac" }, { line_position: "2", line_desc: "\u00d6lfilter", line_quantity: "1", line_unit: "St\u00fcck", line_unit_price: "9,90 \u20ac", line_total: "9,90 \u20ac" }, { line_position: "3", line_desc: "Lange Beispielbeschreibung zur Kontrolle von Textbreite und Umbruch", line_quantity: "1", line_unit: "St\u00fcck", line_unit_price: "129,00 \u20ac", line_total: "129,00 \u20ac" }, { line_position: "4", line_desc: "Arbeitszeit \u2013 Diagnose und Fehlersuche", line_quantity: "2,50", line_unit: "Std.", line_unit_price: "79,00 \u20ac", line_total: "197,50 \u20ac" }];
